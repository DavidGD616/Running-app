...
void main() {
  ...
  test('activities provider reloads persisted activities after recreation', () async {
    ...
    expect(container.read(activitiesProvider), hasLength(2));
    expect(container.read(activitiesProvider).first.id, 'activity-linked');
    expect(
    ...
    final reloaded = recreated.read(activitiesProvider);
    expect(reloaded, hasLength(2));
    expect(reloaded.first.id, 'activity-linked');
    expect(recreated.read(activitiesByLinkedSessionIdProvider('w4-tue')),
        hasLength(1));
    expect(
