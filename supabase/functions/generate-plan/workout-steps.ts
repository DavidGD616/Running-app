...
type StepKind = 'warmUp' | 'work' | 'recovery' | 'coolDown' | 'repeat' | 'stride';
...
export function buildWorkoutSteps(
  ...
  const strideRecoverySeconds = positiveInt(session.strideRecoverySeconds) ?? 75;
  ...
  if (session.intervalReps && session.intervalRepDistanceMeters) {
    ...
    steps.push({
      kind: 'work',
      target,
      durationMs: mainDurationMinutes != null ? mainDurationMinutes * 60_000 : null,
...
function mainWorkMinutes(
  ...
  return Math.max(1, session.durationMinutes - warmUp - coolDown - extraMinutes);
}
