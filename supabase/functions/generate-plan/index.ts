...
import { buildCandidatePlan } from "./candidate-builder.ts";
...
Deno.serve(async (req) => {
  ...
  const stravaSnapshotSource = professionalInput?.stravaCoachingProfile ??
    (isRecord(sanitizedGenerationProfile.fitness?.stravaCoachingProfile)
      ...
      : undefined);
  const candidatePlan = await buildCandidatePlan({
    generationProfileWithPlanStartDate,
  ...
  if (candidatePlan instanceof Response) {
  ...
  const planJson = {
    ...candidatePlan,
    id: versionId,
    currentWeekNumber: candidatePlan.currentWeekNumber ?? 1,
  };
